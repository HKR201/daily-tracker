      ...
      body: p.isLoading ? const Center(child: CircularProgressIndicator()) : Column(
        children: [
          ...
          GestureDetector(
            ...
            child: Text(_showFullBalance ? '${currencyFormat.format(p.totalBalance)} Ks' : '${p.formatLakh(p.totalBalance)} Ks', style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: p.totalBalance < 0 ? Colors.redAccent : Colors.blueAccent)),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        bottom: TabBar(controller: _tab, isScrollable: false, labelColor: Colors.blueAccent, unselectedLabelColor: Colors.grey, indicatorColor: Colors.blueAccent, tabs: const [Tab(text: 'Monthly'), Tab(text: 'Yearly'), Tab(text: 'More')])
      ),
      body: Stack(children: [
        TabBarView(controller: _tab, children: [_buildAccordion(p, 'Monthly'), _buildAccordion(p, 'Yearly'), const SettingsView()]),
  ...
  Widget _buildAssetBox(TrackerProvider p) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Stack(alignment: Alignment.center, children: [const Text('စုစုပေါင်း ပိုင်ဆိုင်မှု', textAlign: TextAlign.center, style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.bold, fontSize: 16)), Align(alignment: Alignment.centerRight, child: Icon(_isAssetsHidden ? Icons.visibility_off : Icons.visibility, color: Colors.blueAccent, size: 22))]),
            if (!_isAssetsHidden) ...[
              const SizedBox(height: 15), Text('${p.formatLakh(p.totalAssets)} Ks', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.blueAccent)), const SizedBox(height: 10), const Divider(height: 1), const SizedBox(height: 15),
              Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: p.wallets.map((w) => Column(children: [Text(w.name, style: const TextStyle(fontSize: 13, color: Colors.grey)), const SizedBox(height: 5), Text(p.formatLakh(w.amount), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15))])).toList()),
            ]
          ]
        )
      ),
...
class _SettingsViewState extends State<SettingsView> {
  ...
  void _showRenameDialog(String title, String initialValue, Function(String) onSave) {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: TextField(autofocus: true, controller: TextEditingController(text: initialValue), decoration: const InputDecoration(border: OutlineInputBorder()), onChanged: (val) => newVal = val),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel', style: TextStyle(color: Colors.grey))),
        ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent), onPressed: () { if (newVal.trim().isNotEmpty) { onSave(newVal.trim()); Navigator.pop(ctx); } }, child: const Text('Save', style: TextStyle(color: Colors.white))),
      ],
    ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        const Text('Display & Format', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 10),
        Container(
            ...
            children: [
              ...
              ListTile(
                ...
                trailing: DropdownButton<ThemeMode>(
                  ...
                  items: const [
                    DropdownMenuItem(value: ThemeMode.system, child: Text('System (ဖုန်းအတိုင်း)')),
                    DropdownMenuItem(value: ThemeMode.light, child: Text('Light (အလင်း)')),
                    DropdownMenuItem(value: ThemeMode.dark, child: Text('Dark (အမှောင်)')),
                  ],
                  onChanged: (val) { if (val != null) provider.setThemeMode(val); },
                ),
              )
            ]
          )
        ...
        const Text('Personalization (နာမည်များ ပြင်ဆင်ရန်)', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 10),
        Container(decoration: BoxDecoration(color: isDark ? Colors.blueGrey.withOpacity(0.2) : Colors.white, borderRadius: BorderRadius.circular(15)), child: Column(children: [ListTile(leading: const Icon(Icons.title, color: Colors.blueAccent), title: const Text('Rename Daily Hub', style: TextStyle(fontWeight: FontWeight.bold)), trailing: const Icon(Icons.edit, size: 18), onTap: () => _showRenameDialog('Rename Daily Hub', provider.hubTitle, (v) => provider.updateHubTitle(v))), const Divider(height: 1), ListTile(leading: const Icon(Icons.title, color: Colors.blueAccent), title: const Text('Rename Vault', style: TextStyle(fontWeight: FontWeight.bold)), trailing: const Icon(Icons.edit, size: 18), onTap: () => _showRenameDialog('Rename Vault', provider.vaultTitle, (v) => provider.updateVaultTitle(v))), const Divider(height: 1), ExpansionTile(leading: const Icon(Icons.account_balance_wallet, color: Colors.blueAccent), title: const Text('Rename Pockets (အိတ်ကပ်များ)', style: TextStyle(fontWeight: FontWeight.bold)), children: provider.wallets.map((w) => ListTile(title: Text(w.name), trailing: const Icon(Icons.edit, size: 18), onTap: () => _showRenameDialog('Rename Pocket', w.name, (v) => provider.renameWallet(w.id!, v)))).toList())])),
        const SizedBox(height: 30),
        const Text('Cloud Backup & Security', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 10),
        Container(
          decoration: BoxDecoration(color: isDark ? Colors.blueGrey.withOpacity(0.2) : Colors.white, borderRadius: BorderRadius.circular(15)),
          child: Column(
            children: [
              ListTile(leading: const Icon(Icons.cloud_upload, color: Colors.blueAccent), title: const Text('Backup to Google Drive', style: TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('Last synced: ${provider.lastSyncTime}'), trailing: _isSyncing ? const CircularProgressIndicator() : const Icon(Icons.chevron_right), onTap: () async { setState(() => _isSyncing = true); await _driveService.backupDatabase(); provider.updateSyncTime(); setState(() => _isSyncing = false); }), const Divider(height: 1),
              ListTile(leading: const Icon(Icons.cloud_download, color: Colors.green), title: const Text('Restore from Google Drive', style: TextStyle(fontWeight: FontWeight.bold)), trailing: _isSyncing ? const CircularProgressIndicator() : const Icon(Icons.chevron_right), onTap: () async { setState(() => _isSyncing = true); await _driveService.restoreDatabase(); provider.loadAllData(); setState(() => _isSyncing = false); }), const Divider(height: 1),
              ListTile(leading: const Icon(Icons.delete_forever, color: Colors.redAccent), title: const Text('Reset Cloud Backup', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.redAccent)), trailing: _isResetting ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.redAccent, strokeWidth: 2)) : const Icon(Icons.chevron_right, color: Colors.redAccent), onTap: () async {
                bool? confirm = await showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text('Are you sure?', style: TextStyle(fontWeight: FontWeight.bold)), content: const Text('This will permanently delete your cloud backup.\n\nYou will need to choose your Google account again to verify.'), actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel', style: TextStyle(color: Colors.grey))), ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent), onPressed: () => Navigator.pop(ctx, true), child: const Text('Reset', style: TextStyle(color: Colors.white)))]));
                if (confirm == true) {
                  setState(() => _isResetting = true);
                  bool success = await _driveService.resetCloudBackup();
                  if (success) { final prefs = await SharedPreferences.getInstance(); await prefs.setString('lastSyncTime', 'Never'); provider.lastSyncTime = 'Never'; provider.loadAllData(); }
                  setState(() => _isResetting = false);
                  if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Cloud Backup has been reset.' : 'Failed to reset backup.')));
                }
              }),
            ],
          ),
